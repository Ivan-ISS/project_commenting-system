class Favorites {
    ...
    public filterFavorites() {
        ...
        for (let j = 0; j < arrComments.length; j++) {
            /* console.log(arrComments[j].querySelectorAll('.comment-in__answer-comment')); */
        ...
        /* for (let i = 0; i < arrComments.length; i++) {
            arrComments[i].addEventListener('click', (event) => {
                this._commentingSystem.commentsStorage.insertCommentHistory();
                this._commentingSystem.sorting.sortByDate();
